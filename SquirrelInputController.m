...
- (BOOL)processKey:(int)rime_keycode
  ...
  if (panel.tabular && !rime_modifiers &&
    ...
    if (newIndex != NSNotFound) {
      ...
      return (BOOL)rime_get_api()->highlight_candidate(_session, newIndex) || panel.visible;
...
NSString *getOptionLabel(RimeSessionId session, const char *option, Bool state) {
  ...
  if (short_label.str && short_label.length >= strlen(short_label.str)) {
    ...
    RimeStringSlice long_label = 
      rime_get_api()->get_state_label_abbreviated(session, option, state, False);
...
- (void)rimeUpdate {
  ...
  if (rime_get_api()->get_context(_session, &ctx)) {
    ...
    BOOL didCompose = didCommit ||
      ![[composedText stringByReplacingOccurrencesOfString:@" " withString:@""]
        isEqualToString:[_composedString stringByReplacingOccurrencesOfString:@" " withString:@""]];
    ...
    if (panel.tabular && !showingStatus) {
      if (numCandidates == 0 || didCompose) {
        ...
        if (!panel.locked && panel.expanded && panel.topRow && pageNum == 0 && highlightedIndex == 0) {
          panel.expanded = NO;
        } else if (!panel.locked && !panel.expanded && pageNum > currentPageNum) {
          panel.expanded = YES;
        }
        if (panel.expanded && pageNum > currentPageNum && panel.activePage < 4) {
          panel.activePage = MIN(panel.activePage + pageNum - currentPageNum, finalPage ? 4UL : 3UL);
        } else if (panel.expanded && pageNum < currentPageNum && panel.activePage > 0) {
          panel.activePage = MAX(panel.activePage + pageNum - currentPageNum, pageNum == 0 ? 0UL : 1UL);
        }
      }
      highlightedIndex += pageSize * panel.activePage;
    }
    _currentIndex = numCandidates == 0 ? NSNotFound : highlightedIndex + pageSize * (pageNum - panel.activePage);
      ...
      if (_inlinePreedit) {
        if (end <= caretPos && caretPos < length) {
          candidatePreviewText = [candidatePreviewText stringByAppendingString:
                                  [preeditText substringWithRange:
                                   NSMakeRange(caretPos, length - caretPos)]];
        }
    ...
    if (numCandidates > 0 && panel.expanded && panel.activePage < 4) {
