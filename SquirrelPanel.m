...
- (void)superscriptRange:(NSRange)range {
  [self enumerateAttribute:NSFontAttributeName
    ...
    [self addAttributes:@{NSFontAttributeName : font,
            (id)kCTBaselineClassAttributeName : (id)kCTBaselineClassIdeographicHigh,
...
- (void)subscriptRange:(NSRange)range {
  [self enumerateAttribute:NSFontAttributeName
    ...
    NSFont *font = [NSFont fontWithDescriptor:value.fontDescriptor
                                         size:floor(value.pointSize * 0.55)];
    [self addAttributes:@{NSFontAttributeName : font,
            (id)kCTBaselineClassAttributeName : (id)kCTBaselineClassIdeographicLow,
                   NSSuperscriptAttributeName : @(-1)}
                  range:subRange];
  }];
}
...
- (void)updateSeperatorAndSymbolAttrs {
  ...
  if (_tabular) {
    ...
    attmCompress.image = [NSImage imageNamed:@"Symbols/arrow.down.and.line.horizontal.and.arrow.up"];
    ...
    attmExpand.image = [NSImage imageNamed:@"Symbols/arrow.up.and.line.horizontal.and.arrow.down"];
    ...
    _expanderWidth = MAX(MAX(_symbolCompress.size.width, _symbolExpand.size.width),
                         _symbolLock.size.width);
  } else if (_showPaging) {
...
@property(nonatomic) NSRange *candidateRanges;
...
- (void)drawViewWithInsets:(NSEdgeInsets)alignmentRectInsets
  ...
  _tabularPositions = numCandidates > 0 ? new SquirrelTabularPosition[numCandidates] : NULL;
  _expanderPath = nil;
  _deleteBackPath = nil;
  _candidatePaths = numCandidates > 0 ? [[NSMutableArray alloc] initWithCapacity:numCandidates] : nil;
  _pagingPaths = pagingRange.length > 0 || self.expanded ? [[NSMutableArray alloc] initWithCapacity:2] : nil;
...
- (void)updateLayer {
  ...
  if (candidateBlockRange.length > 0) {
    ...
    candidateBlockPath = squirclePath(candidateBlockVertices, 4, MIN(theme.highlightedCornerRadius,
    ...
    if (theme.linear) {
      ...
      for (NSUInteger i = 0; i < _numCandidates; ++i) {
        ...
        if (theme.tabular) {
          if (self.expanded && i / theme.pageSize == _highlightedIndex / theme.pageSize) {
            if (i % theme.pageSize == 0) {
              ...
              activePagePath = squirclePath(activePageVertices, 4, MIN(theme.highlightedCornerRadius,
          ...
          if (ABS(bottomEdge - gridOriginY) > 2) {
            ...
            if (ABS(bottomEdge - NSMaxY(_candidateBlock)) > 2) { // horizontal border except for the last row
        ...
        if (NSIsEmptyRect(bodyRect) && !NSIsEmptyRect(leadingRect) &&
          ...
          [candidatePath appendBezierPath:squirclePath(trailingVertices, 4, cornerRadius)];
        } else {
          numVert = MIN(8, MAX(4, numVert));
          NSPoint multilineVertices[numVert];
          multilineRectVertices(leadingRect, bodyRect, trailingRect, multilineVertices);
          candidatePath = squirclePath(multilineVertices, numVert, cornerRadius);
        }
  ...
  if (theme.tabular && candidateBlockRange.length > 0) {
    NSRect expanderRect = [self blockRectForRange:NSMakeRange(_textStorage.length - 1, 1)];
    ...
    expanderRect = [self backingAlignedRect:NSIntersectionRect(expanderRect, backgroundRect)
                                    options:NSAlignAllEdgesNearest];
    ...
    _expanderPath = squirclePath(expanderVertices, 4, MIN(theme.highlightedCornerRadius,
                                                          theme.paragraphStyle.minimumLineHeight * 0.5));
    if (theme.showPaging && self.expanded && _tabularPositions[_numCandidates - 1].row > 0) {
      ...
      CGFloat width = MIN(theme.paragraphStyle.minimumLineHeight, NSWidth(_pagingBlock));
      CGFloat height = MIN(width, NSHeight(_pagingBlock) * 0.5);
      NSRect pageUpRect = NSMakeRect(NSMinX(_pagingBlock), NSMidY(_pagingBlock) - height, width, height);
      NSRect pageDownRect = NSMakeRect(NSMinX(_pagingBlock), NSMidY(_pagingBlock),  width, height);
      ...
      [scrollerPath moveToPoint:NSMakePoint(NSMinX(pageUpRect) + ceil(width * 0.2),
                                            NSMaxY(pageUpRect) - ceil(width * 0.2))];
      [scrollerPath lineToPoint:NSMakePoint(NSMidX(pageUpRect),
                                            NSMaxY(pageUpRect) - ceil(width * 0.4))];
      [scrollerPath lineToPoint:NSMakePoint(NSMaxX(pageUpRect) - ceil(width * 0.2),
                                            NSMaxY(pageUpRect) - ceil(width * 0.2))];
      [scrollerPath moveToPoint:NSMakePoint(NSMinX(pageDownRect) + ceil(width * 0.2),
      ...
      [scrollerPath lineToPoint:NSMakePoint(NSMaxX(pageDownRect) - ceil(width * 0.2),
                                            NSMinY(pageDownRect) + ceil(width * 0.2))];
    }
    ...
    pageDownRect = [self backingAlignedRect:pageDownRect
                                    options:NSAlignAllEdgesNearest];
    CGFloat cornerRadius = MIN(theme.highlightedCornerRadius, MIN(NSWidth(pageDownRect),
                                                                  NSHeight(pageDownRect)) * 0.5);
    NSPoint pageUpVertices[4], pageDownVertices[4];
    rectVertices(pageUpRect, pageUpVertices);
    rectVertices(pageDownRect, pageDownVertices);
    _pagingPaths[0] = squirclePath(pageUpVertices, 4, cornerRadius);
  ...
  if ((!NSIsEmptyRect(_preeditBlock) || !NSIsEmptyRect(_pagingBlock) ||
       _expanderPath) && theme.preeditBackColor) {
  ...
  if (scrollerPath) {
    ...
    scrollerLayer.lineWidth = ceil([theme.pagingAttrs[NSFontAttributeName] pointSize] * 0.05);
    scrollerLayer.strokeColor = [theme.pagingAttrs[NSForegroundColorAttributeName] CGColor];
...
- (SquirrelIndex)getIndexFromMouseSpot:(NSPoint)spot {
  ...
  if (NSPointInRect(point, self.bounds)) {
    ...
    if (NSPointInRect(point, _preeditBlock)) {
      return [_deleteBackPath.bezierPathByFlatteningPath
              containsPoint:point] ? kBackSpaceKey : kCodeInputArea;
    }
    if ([_expanderPath.bezierPathByFlatteningPath
         containsPoint:point]) {
      return kExpandButton;
    } else if (_pagingPaths.count > 0) {
      if ([_pagingPaths[0].bezierPathByFlatteningPath
           containsPoint:point]) {
        return kPageUpKey;
      }
      if ([_pagingPaths[1].bezierPathByFlatteningPath
           containsPoint:point]) {
        return kPageDownKey;
      }
    }
    for (NSUInteger i = 0; i < _candidatePaths.count; ++i) {
      if ([_candidatePaths[i].bezierPathByFlatteningPath
           containsPoint:point]) {
        return i;
      }
    }
...
- (BOOL)topRow {
  return _view.tabularPositions ? _view.tabularPositions[_highlightedIndex].row == 0 : YES;
}
...
- (void)setActivePage:(NSUInteger)activePage {
  if (_view.currentTheme.tabular && _view.expanded && _activePage != activePage) {
    _activePage = MAX(MIN(activePage, 4UL), 0UL);
  }
}
...
- (NSUInteger)candidateIndexOnDirection:(SquirrelIndex)arrowKey {
  if (!self.tabular || _candidates.count == 0 || _highlightedIndex == NSNotFound) {
    return NSNotFound;
  }
...
- (void)sendEvent:(NSEvent *)event {
  ...
  switch (event.type) {
    ...
    case NSEventTypeMouseMoved: {
      ...
      if (cursorIndex >= 0 && cursorIndex < _candidates.count && _highlightedIndex != cursorIndex) {
        ...
        switch (_functionButton) {
          case kPageUpKey:
            ...
            [_toolTip showWithToolTip:NSLocalizedString(_pageNum == 0 ? @"home" : @"page_up", nil) withDelay:!noDelay];
            break;
          case kPageDownKey:
            if (theme.tabular) {
              [_view.textStorage addAttributes:theme.pagingAttrs
            ...
            [_toolTip showWithToolTip:NSLocalizedString(_finalPage ? @"end" : @"page_down", nil) withDelay:!noDelay];
            break;
          case kExpandButton:
            [_view.textStorage addAttributes:theme.pagingHighlightedAttrs
                                       range:NSMakeRange(_view.textStorage.length - 1, 1)];
            ...
            [_toolTip showWithToolTip:NSLocalizedString(_locked ? @"unlock" : _view.expanded ? @"compress" : @"expand", nil) withDelay:!noDelay];
            break;
          case kBackSpaceKey:
            ...
            BOOL caretAtHome = _caretPos == NSNotFound || (_caretPos == _selRange.location && _selRange.location == 1);
            cursorIndex = caretAtHome ? kEscapeKey : kBackSpaceKey;
            [_toolTip showWithToolTip:NSLocalizedString(caretAtHome ? @"escape" : @"delete", nil) withDelay:!noDelay];
            break;
        }
...
- (void)updateDisplayParameters {
  ...
  if (theme.tabular) {
    ...
    _textWidthLimit = floor((_textWidthLimit - theme.expanderWidth) / tabInterval) * tabInterval + theme.expanderWidth;
  }
...
- (BOOL)shouldBreakLineInsideRange:(NSRange)range {
  ...
  if (self.tabular) {
    maxTextWidth -= _view.currentTheme.expanderWidth + _view.currentTheme.separatorWidth;
  }
...
- (void)updateContents {
  ...
  _view.candidateRanges = _candidates.count > 0 ? new NSRange[_candidates.count] : NULL;
  ...
  if (_preedit) {
    ...
    if (_selRange.length > 0) {
      ...
      if (_selRange.location > 0) {
        [preeditLine addAttribute:NSKernAttributeName
                            value:@(kerning * 2)
                            range:NSMakeRange(highlightedPreeditStart - 1, 1)];
      }
      if (NSMaxRange(_selRange) < _preedit.length) {
        [preeditLine addAttribute:NSKernAttributeName
                            value:@(kerning * 2)
                            range:NSMakeRange(preeditLine.length - 1, 1)];
      }
    }
    if (NSMaxRange(_selRange) < _preedit.length) {
      [preeditLine appendAttributedString:[[NSAttributedString alloc]
                                           initWithString:[_preedit substringFromIndex:NSMaxRange(_selRange)]
                                           attributes:theme.preeditAttrs]];
    }
    [preeditLine appendAttributedString:[[NSAttributedString alloc]
    ...
    if (_caretPos != NSNotFound) {
      [preeditLine addAttribute:NSVerticalGlyphFormAttributeName value:@(NO)
                          range:NSMakeRange(_caretPos - (_caretPos < NSMaxRange(_selRange)), 1)];
    }
  ...
  for (NSUInteger idx = 0; idx < _candidates.count; ++idx) {
    ...
    NSMutableAttributedString *item = idx == _highlightedIndex
    ...
    if (_comments[idx].length > 0) {
      [item replaceCharactersInRange:commentField
                          withString:[@" " stringByAppendingString:_comments[idx]]];
    } else {
    ...
    if (theme.linear && ceil(item.size.width) > textWidthLimit) {
      ...
      _view.candidateRanges[idx] = NSMakeRange(lineStart, item.length);
      lineStart = text.length;
    } else {
  ...
  if (theme.tabular) {
    ...
    CGFloat expanderPosition = ceil((maxLineLength - theme.expanderWidth) / tabInterval) * tabInterval;
...
alignDelete:
  ...
  CGFloat bottomMargin = _candidates.count > 0 && (theme.linear || !theme.showPaging) ? floor(theme.linespace * 0.5) : 0.0;
  ...
  self.animationBehavior = _caretPos == NSNotFound ? NSWindowAnimationBehaviorUtilityWindow
