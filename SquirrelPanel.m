...
- (CGFloat)annotateRubyInRange:(NSRange)range
  ...
  [regex enumerateMatchesInString:self.mutableString
    ...
    if ([self attributedSubstringFromRange:NSMakeRange(0, NSMaxRange(baseRange))].size.width > maxLength - 0.1) {
      ...
      rubyLineHeight = fmax(rubyLineHeight, ceil(height * 0.5));
...
- (void)updateSeperatorAndSymbolAttrs {
  ...
  if (_tabular) {
    ...
    _expanderWidth = fmax(fmax(ceil(_symbolCompress.size.width), ceil(_symbolExpand.size.width)),
...
- (void)updateCandidateFormats {
  ...
  NSFont *substituteFont = CFBridgingRelease(CTFontCreateForString(
  ...
  if ([substituteFont isNotEqualTo:labelFont]) {
    ...
    NSMutableDictionary *labelHighlightedAttrs = _labelHighlightedAttrs.mutableCopy;
    ...
    if (_linear) {
      ...
      NSMutableDictionary *pagingHighlightAttrs = _pagingHighlightedAttrs.mutableCopy;
  ...
  candidateRange = [candidateFormat rangeOfString:@"%@" options:NSLiteralSearch];
  ...
  candidateRange = [format.mutableString rangeOfString:@"%@" options:NSLiteralSearch];
  commentRange = NSMakeRange(NSMaxRange(candidateRange),
                             format.length - NSMaxRange(candidateRange));
  if (commentRange.length > 0) {
    [format replaceCharactersInRange:commentRange
                          withString:[kTipSpecifier stringByAppendingString:
                                      [format.mutableString substringWithRange:commentRange]]];
    [highlightedFormat replaceCharactersInRange:commentRange
                                     withString:[kTipSpecifier stringByAppendingString:
  ...
  NSMutableArray *candidateFormats = [[NSMutableArray alloc]
                                      initWithCapacity:labels.count];
  NSMutableArray *candidateHighlightedFormats = [[NSMutableArray alloc]
                                                 initWithCapacity:labels.count];
  enumRange = [format.mutableString rangeOfString:@"%c" options:NSLiteralSearch];
  for (NSString *label in labels) {
    NSMutableAttributedString *newFormat = format.mutableCopy;
    NSMutableAttributedString *newHighlightedFormat = highlightedFormat.mutableCopy;
...
- (BOOL)      layoutManager:(NSLayoutManager *)layoutManager
  ...
  CGFloat lineHeightDelta = lineFragmentUsedRect->size.height -
                            rulerAttrs.minimumLineHeight - rulerAttrs.lineSpacing;
  ...
  *baselineOffset = floor(lineFragmentUsedRect->origin.y - lineFragmentRect->origin.y + rulerAttrs.minimumLineHeight * 0.5 +
...
- (BOOL)      textLayoutManager:(NSTextLayoutManager *)textLayoutManager
  ...
  NSInteger charIndex = [contentStorage
                         ...
                         toLocation:location];
  return charIndex <= 1 || [contentStorage.textStorage.mutableString
...
- (void)  setPreeditRange:(NSRange)preeditRange
  ...
  NSRect mirrorPreeditBlock = NSOffsetRect(_preeditBlock, 0, NSHeight(self.bounds) - NSHeight(_preeditBlock) * 2);
...
static NSBezierPath *squirclePath(NSPointArray vertices, NSInteger numVert, CGFloat radius) {
  if (vertices == NULL) {
    return nil;
  }
...
static void multilineRectVertices(NSRect leadingRect, NSRect bodyRect, NSRect trailingRect, NSPointArray vertices) {
  switch ((NSIsEmptyRect(leadingRect) << 2) + (NSIsEmptyRect(bodyRect) << 1) + (NSIsEmptyRect(trailingRect) << 0)) {
    case 0b011:
...
- (CAShapeLayer *)getFunctionButtonLayer {
  ...
  if (!NSIsEmptyRect(buttonRect) && buttonColor) {
    CGFloat cornerRadius = fmin(theme.highlightedCornerRadius, NSHeight(buttonRect) * 0.5);
...
- (void)updateLayer {
  ...
  CGFloat innerCornerRadius = fmax(fmin(theme.highlightedCornerRadius, NSHeight(backgroundRect) * 0.5),
                                  outerCornerRadius - fmin(theme.borderInset.width, theme.borderInset.height));
  ...
  NSBezierPath *backgroundPath = squirclePath(backgroundVertices, 4, innerCornerRadius);
  ...
  if (_numCandidates > 0) {
    NSRange endRange = theme.linear && _pagingRange.length > 0 ? _pagingRange : _candidateRanges[_numCandidates - 1];
    candidateBlockRange = NSIntersectionRange(NSUnionRange(_candidateRanges[0], endRange), visibleRange);
  } else {
  ...
  if (preeditRange.length > 0) {
    ...
    if (highlightedPreeditRange.length > 0 && theme.highlightedPreeditBackColor) {
      ...
      innerBox.size.width = backgroundRect.size.width - theme.separatorWidth + kerning * 2;
      ...
      if (NSIsEmptyRect(bodyRect) && !NSIsEmptyRect(leadingRect) && !NSIsEmptyRect(trailingRect) &&
        ...
        [highlightedPreeditPath appendBezierPath:squirclePath(trailingVertices, 4, cornerRadius)];
      } else {
        numVert = numVert > 8 ? 8 : numVert < 4 ? 4 : numVert;;
        NSPoint multilineVertices[numVert];
        multilineRectVertices(leadingRect, bodyRect, trailingRect, multilineVertices);
        highlightedPreeditPath = squirclePath(multilineVertices, numVert, cornerRadius);
      }
    }
    _deleteBackRect = [self blockRectForRange:NSMakeRange(NSMaxRange(_preeditRange) - 1, 1)];
  ...
  if (candidateBlockRange.length > 0) {
    ...
    candidateBlockPath = squirclePath(candidateBlockVertices, 4, fmin(theme.highlightedCornerRadius,
    ...
    if (theme.linear) {
      ...
      for (NSUInteger i = 0; i < _numCandidates; ++i) {
        ...
        if (!NSIsEmptyRect(bodyRect)) {
          ...
          if (_truncated[i]) {
            ...
            bodyRect.size.width += theme.tabular && NSIsEmptyRect(trailingRect) ? 0.0 : theme.separatorWidth;
          }
          bodyRect.origin.y += _alignmentRectInsets.top;
        ...
        if (theme.tabular) {
          if (self.expanded) {
            if (i % theme.pageSize == 0) {
              ...
              if (sec == _highlightedIndex / theme.pageSize) {
                ...
                activePagePath = squirclePath(activePageVertices, 4, fmin(theme.highlightedCornerRadius,
          ...
          if (fabs(bottomEdge - gridOriginY) > 2) {
             lineNum += i > 0 ? 1 : 0;
            if (fabs(bottomEdge - NSMaxY(_candidateBlock)) > 2) { // horizontal border except for the last line
              [gridPath moveToPoint:NSMakePoint(NSMinX(_candidateBlock) + ceil(theme.separatorWidth * 0.5), bottomEdge)];
              [gridPath lineToPoint:NSMakePoint(NSMaxX(_candidateBlock) - floor(theme.separatorWidth * 0.5), bottomEdge)];
            }
      ...
      NSInteger numVert = (NSIsEmptyRect(_candidateRects[_highlightedIndex * 3]) ? 0 : 4) +
      ...
      if (numVert == 8 && NSMaxX(_candidateRects[_highlightedIndex * 3 + 2]) <
        ...
        rectVertices(_candidateRects[_highlightedIndex * 3 + 2], trailingVertices);
        highlightedCandidatePath = squirclePath(leadingVertices, 4, cornerRadius);
        [highlightedCandidatePath appendBezierPath:squirclePath(trailingVertices, 4, cornerRadius)];
      } else {
        numVert = numVert > 8 ? 8 : numVert < 4 ? 4 : numVert;
        NSPoint multilineVertices[numVert];
        multilineRectVertices(_candidateRects[_highlightedIndex * 3],
                              ...
                              multilineVertices);
        highlightedCandidatePath = squirclePath(multilineVertices, numVert, cornerRadius);
      }
    } else { // stacked layout
      ...
      highlightedCandidatePath = squirclePath(candidateVertices, 4, cornerRadius);
    }
  }
  ...
  if (theme.tabular && candidateBlockRange.length > 0) {
    _expanderRect = [self blockRectForRange:NSMakeRange(_textStorage.length - 1, 1)];
    ...
    _expanderRect.origin.y += _alignmentRectInsets.top - ceil(theme.linespace * 0.5);
    _expanderRect = [self backingAlignedRect:NSIntersectionRect(_expanderRect, backgroundRect)
                                    options:NSAlignAllEdgesNearest];
    if (theme.showPaging && self.expanded && _tabularIndices[_numCandidates - 1].lineNum > 0) {
      ...
      CGFloat width = fmin(theme.paragraphStyle.minimumLineHeight, NSWidth(_pagingBlock));
      ...
      pageUpPath = [NSBezierPath bezierPathWithOvalInRect:NSInsetRect(_pageUpRect, width * 0.2, width * 0.2)];
      [pageUpPath moveToPoint:NSMakePoint(NSMinX(_pageUpRect) + ceil(width * 0.325),
                                          NSMaxY(_pageUpRect) - ceil(width * 0.4))];
      [pageUpPath lineToPoint:NSMakePoint(NSMidX(_pageUpRect),
                                          NSMinY(_pageUpRect) + ceil(width * 0.4))];
      [pageUpPath lineToPoint:NSMakePoint(NSMaxX(_pageUpRect) - ceil(width * 0.325),
                                          NSMaxY(_pageUpRect) - ceil(width * 0.4))];
      pageDownPath = [NSBezierPath bezierPathWithOvalInRect:NSInsetRect(_pageDownRect, width * 0.2, width * 0.2)];
      [pageDownPath moveToPoint:NSMakePoint(NSMinX(_pageDownRect) + ceil(width * 0.325),
                                            NSMinY(_pageDownRect) + ceil(width * 0.4))];
      [pageDownPath lineToPoint:NSMakePoint(NSMidX(_pageDownRect),
                                            NSMaxY(_pageDownRect) - ceil(width * 0.4))];
      [pageDownPath lineToPoint:NSMakePoint(NSMaxX(_pageDownRect) - ceil(width * 0.325),
                                            NSMinY(_pageDownRect) + ceil(width * 0.4))];
    }
  ...
  if (pageUpPath && pageDownPath) {
    ...
    pageUpLayer.lineWidth = ceil([theme.pagingAttrs[NSFontAttributeName] pointSize] * 0.05);
    NSDictionary *pageUpAttrs = _functionButton == kPageUpKey || _functionButton == kHomeKey ? theme.preeditHighlightedAttrs : theme.preeditAttrs;
    pageUpLayer.strokeColor = [pageUpAttrs[NSForegroundColorAttributeName] CGColor];
    ...
    pageDownLayer.lineWidth = ceil([theme.pagingAttrs[NSFontAttributeName] pointSize] * 0.05);
    NSDictionary *pageDownAttrs = _functionButton == kPageDownKey || _functionButton == kEndKey ? theme.preeditHighlightedAttrs : theme.preeditAttrs;
    pageDownLayer.strokeColor = [pageDownAttrs[NSForegroundColorAttributeName] CGColor];
...
- (void)sendEvent:(NSEvent *)event {
  ...
  switch (event.type) {
    ...
    case NSEventTypeLeftMouseUp:
      if (event.clickCount == 1 && cursorIndex != NSNotFound) {
        if (cursorIndex == _highlightedIndex) {
          [self.inputController performAction:kSELECT
                                      onIndex:cursorIndex + _indexRange.location];
        } else if (cursorIndex == _functionButton) {
          if (cursorIndex == kExpandButton) {
            if (_locked) {
              [self setLock:NO];
              [_view.textStorage replaceCharactersInRange:NSMakeRange(_view.textStorage.length - 1, 1)
    ...
    case NSEventTypeRightMouseUp:
      if (event.clickCount == 1 && cursorIndex != NSNotFound) {
        if (cursorIndex == _highlightedIndex) {
          [self.inputController performAction:kDELETE
                                      onIndex:cursorIndex + _indexRange.location];
        } else if (cursorIndex == _functionButton) {
          switch (_functionButton) {
            case kPageUpKey:
            ...
            case kExpandButton:
              ...
              [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                        value:theme.preeditHighlightedAttrs[NSForegroundColorAttributeName]
    ...
    case NSEventTypeMouseMoved: {
      ...
      if (cursorIndex >= 0 && cursorIndex < _indexRange.length && _highlightedIndex != cursorIndex) {
        ...
        if (theme.linear && _view.truncated[cursorIndex]) {
          [_toolTip showWithToolTip:[_view.textStorage.mutableString substringWithRange:_view.candidateRanges[cursorIndex]] withDelay:NO];
        } else if (noDelay) {
...
- (void)highlightCandidate:(NSUInteger)highlightedIndex {
  ...
  for (NSUInteger i = 0; i < theme.pageSize; ++i) {
    ...
    if ((_sectionNum != prevSectionNum || prevIndex == prevHighlightedIndex) && prevIndex < _indexRange.length) {
      ...
      NSRange prevTextRange = [[_view.textStorage.mutableString substringWithRange:prevRange]
      ...
      NSColor *labelColor = [theme.labelAttrs[NSForegroundColorAttributeName]
                             blendedColorWithFraction:prevIndex == prevHighlightedIndex && _sectionNum == prevSectionNum ? 0.0 : 0.5
    ...
    if ((_sectionNum != prevSectionNum || newIndex == _highlightedIndex) && newIndex < _indexRange.length) {
      ...
      NSColor *labelColor = (newIndex == _highlightedIndex ? theme.labelHighlightedAttrs : theme.labelAttrs)
                            [NSForegroundColorAttributeName];
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:labelColor
                                range:NSMakeRange(newRange.location, newTextRange.location)];
      NSColor *textColor = (newIndex == _highlightedIndex ? theme.highlightedAttrs : theme.attrs)
                            [NSForegroundColorAttributeName];
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:textColor
                                range:NSMakeRange(newRange.location + newTextRange.location, newTextRange.length)];
      NSColor *commentColor = (newIndex == _highlightedIndex ? theme.commentHighlightedAttrs : theme.commentAttrs)
                              [NSForegroundColorAttributeName];
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:commentColor
...
- (void)highlightFunctionButton:(SquirrelIndex)functionButton
  ...
  switch (_functionButton) {
    case kPageUpKey:
      if (!theme.tabular) {
        [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                  value:theme.pagingHighlightedAttrs[NSForegroundColorAttributeName]
                                  range:NSMakeRange(_view.pagingRange.location, 1)];
      }
...
- (void)show {
  ...
  if (theme.rememberSize && _statusMessage == nil) {
    if (theme.lineLength == 0 && (theme.vertical
        ? (sweepVertical ? (NSMinY(_IbeamRect) - fmax(NSWidth(maxContentRect), _maxSize.width) - insets.right < NSMinY(screenRect))
        ...
        : (sweepVertical ? (NSMinX(_IbeamRect) - kOffsetGap - NSWidth(screenRect) * textWidthRatio - insets.left - insets.right >= NSMinX(screenRect))
                         : (NSMaxX(_IbeamRect) + fmax(NSWidth(maxContentRect), _maxSize.width) + insets.right > NSMaxX(screenRect))))) {
    ...
    CGFloat textHeight = fmax(NSHeight(maxContentRect), _maxSize.height) + insets.top + insets.bottom;
  ...
  if (NSMaxX(windowRect) > NSMaxX(screenRect)) {
    windowRect.origin.x = (_initPosition && sweepVertical ? fmin(NSMinX(_IbeamRect) - kOffsetGap, NSMaxX(screenRect)) : NSMaxX(screenRect)) - NSWidth(windowRect);
  }
  if (NSMinX(windowRect) < NSMinX(screenRect)) {
    windowRect.origin.x = _initPosition && sweepVertical ? fmax(NSMaxX(_IbeamRect) + kOffsetGap, NSMinX(screenRect)) : NSMinX(screenRect);
  }
  if (NSMinY(windowRect) < NSMinY(screenRect)) {
    windowRect.origin.y = _initPosition && !sweepVertical ? fmax(NSMaxY(_IbeamRect) + kOffsetGap, NSMinY(screenRect)) : NSMinY(screenRect);
  }
  if (NSMaxY(windowRect) > NSMaxY(screenRect)) {
    windowRect.origin.y = (_initPosition && !sweepVertical ? fmin(NSMinY(_IbeamRect) - kOffsetGap, NSMaxY(screenRect)) : NSMaxY(screenRect)) - NSHeight(windowRect);
  }
...
- (BOOL)shouldBreakLineInsideRange:(NSRange)range {
  ...
  CGFloat maxTextWidth = _textWidthLimit - (theme.tabular ? theme.expanderWidth : 0.0);
...
- (BOOL)shouldUseTabInRange:(NSRange)range
  ...
  if (@available(macOS 12.0, *)) {
    ...
    rangeEndEdge = ceil(NSMaxX([layoutManager lineFragmentUsedRectForGlyphAtIndex:glyphIndex
                                                                   effectiveRange:NULL]));
    containerWidth = ceil(NSMaxX([layoutManager usedRectForTextContainer:
                                  _view.textView.textContainer]));
  }
  if (theme.tabular) {
    containerWidth = ceil((containerWidth - theme.expanderWidth) / (theme.separatorWidth * 2)) *
                      theme.separatorWidth * 2 + theme.expanderWidth;
  }
  *maxLineLength = fmax(*maxLineLength, fmax(fmin(containerWidth, _textWidthLimit), _maxSize.width));
...
- (NSMutableAttributedString *)getPageNumString:(NSUInteger)pageNum {
  ...
  NSImage *pageNumImage = [NSImage imageWithSize:NSMakeSize(height, width)
                                         flipped:YES
    ...
    CGPoint origin = CGPointMake(- pageNumString.size.width / width * NSHeight(dstRect) * 0.5,
                                 - NSWidth(dstRect) * 0.5);
...
- (void)showPreedit:(NSString *)preedit
  ...
  // preedit
  if (preedit) {
    NSMutableAttributedString *preeditLine = [[NSMutableAttributedString alloc]
                                              initWithString:preedit
                                              attributes:theme.preeditAttrs];
    [preeditLine.mutableString appendString:updateCandidates ? kFullWidthSpace : @"\t"];
    ...
    if (updateCandidates) {
      ...
      NSParagraphStyle *rulerStyle = [text attribute:NSParagraphStyleAttributeName
                                             atIndex:0
                                      effectiveRange:NULL];
  ...
  for (NSUInteger idx = 0; idx < indexRange.length; ++idx) {
    ...
    NSMutableAttributedString *item = idx == highlightedIndex
    ...
    if (idx / theme.pageSize != _sectionNum) {
      [item addAttribute:NSForegroundColorAttributeName
                   value:[theme.labelAttrs[NSForegroundColorAttributeName]
                          blendedColorWithFraction:0.5 ofColor:NSColor.clearColor]
                   range:labelRange];
    }
    ...
    if (_comments[idx + indexRange.location].length > 0) {
      [item replaceCharactersInRange:commentField
                          withString:[@" " stringByAppendingString:_comments[idx + indexRange.location]]];
    } else {
    ...
    if (lineStart != text.length) {
      ...
      if (theme.linear && (col == 0 || ceil(item.size.width) > textWidthLimit ||
        ...
        NSRange replaceRange = theme.tabular ? NSMakeRange(separatorStart + separator.length, 0) : NSMakeRange(separatorStart, 1);
    ...
    if (theme.linear && ceil(item.size.width) > textWidthLimit) {
      ...
      _view.candidateRanges[idx] = NSMakeRange(lineStart, text.length - lineStart);
      lineStart = text.length;
    } else {
      _view.truncated[idx] = NO;
      _view.candidateRanges[idx] = NSMakeRange(text.length - item.length, item.length);
    }
  }
  ...
  if (theme.tabular) {
    ...
    paragraphStyleCandidate = theme.paragraphStyle.mutableCopy;
    if ([self shouldUseTabInRange:NSMakeRange(pagingStart - 2, 3) maxLineLength:&maxLineLength]) {
      [text replaceCharactersInRange:NSMakeRange(pagingStart, 0) withString:@"\t"];
      ...
      [paragraphStyleCandidate addTabStop:
       [[NSTextTab alloc] initWithTextAlignment:NSTextAlignmentLeft
                                       location:maxLineLength - theme.expanderWidth
                                        options:@{}]];
    }
...
alignDelete:
  ...
  CGFloat bottomMargin = indexRange.length > 0 && (theme.linear || !theme.showPaging) ? floor(theme.linespace * 0.5) : 0.0;
...
+ (void)updateTheme:(SquirrelTheme *)theme
  ...
  if (appear == darkAppear) {
    for (NSString *option in styleOptions) {
      if ((colorScheme = [config getStringForOption:[NSString stringWithFormat:
                                                     @"style/%@/color_scheme_dark", option]])) {
        break;
      }
    }
  ...
  CGFloat maxFontSize = fmax(fontSize.doubleValue, fmax(commentFontSize.doubleValue, labelFontSize.doubleValue));
  ...
  labelHighlightedAttrs[(id)kCTBaselineClassAttributeName] = (id)kCTBaselineClassIdeographicCentered;
  ...
  pagingHighlightedAttrs[(id)kCTBaselineClassAttributeName] = (id)kCTBaselineClassIdeographicCentered;
  ...
  pagingAttrs[NSForegroundColorAttributeName] = linear && !tabular ? candidateLabelColor : textColor;
  pagingHighlightedAttrs[NSForegroundColorAttributeName] = linear && !tabular ? highlightedCandidateLabelColor : highlightedTextColor;
  ...
  [theme    setCornerRadius:fmin(cornerRadius.doubleValue, lineHeight * 0.5)
    highlightedCornerRadius:fmin(highlightedCornerRadius.doubleValue, lineHeight * 0.5)
           ...
           preeditLinespace:spacing.doubleValue
               ...
               translucency:translucency.doubleValue
                 lineLength:lineLength.doubleValue > 0.1 ? fmax(ceil(lineLength.doubleValue), separatorWidth * 5) : 0.0
