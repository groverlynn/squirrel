  ...
  @autoreleasepool {
    ...
    switch (event.type) {
        ...
        switch (keyCode) {
          case kVK_CapsLock:
            if (!_goodOldCapsLock) {
              ...
              rime_modifiers = ascii_mode ? rime_modifiers | kLockMask
          ...
          case kVK_RightShift:
            release_mask =
                modifiers & NSEventModifierFlagShift
                    ? 0
                    : kReleaseMask |
                          (eventCount - _lastEventCount == 1 ? 0
                                                             : kIgnoredMask);
            handled = [self processKey:rime_keycode
      ...
      case NSEventTypeKeyDown: {
        // NSLog(@"KEYDOWN client: %@, modifiers: 0x%lx, keyCode: %d", sender,
        // modifiers, keyCode); translate mac keydown events to rime keyevents
        int rime_keycode = rime_keycode_from_mac_keycode(keyCode);
        if (rime_keycode == 0) {
        ...
        if (rime_keycode != 0) {
          if ((handled = [self processKey:rime_keycode
            ...
            [self rimeUpdate];
          } else if (_panellessCommitFix &&
                     self.client.markedRange.length > 0) {
            if (rime_keycode == XK_Delete ||
...
- (BOOL)mouseDownOnCharacterIndex:(NSUInteger)index
  ...
  *keepTracking = NO;
  if ((!_inlinePreedit && !_inlineCandidate) || _composedString.length == 0 ||
      ...
      (flags & NSEventModifierFlagDeviceIndependentFlagsMask) != 0) {
    return NO;
  }
...
static void set_CapsLock_LED_state(bool target_state) {
  io_service_t ioService = IOServiceGetMatchingService(
...
- (BOOL)processKey:(int)rime_keycode
  ...
  if (is_vertical !=
      rime_get_api_stdbool()->get_option(_session, "_vertical")) {
  ...
  bool handled = rime_get_api_stdbool()->process_key(_session, rime_keycode,
                                                     rime_modifiers);
  ...
  if (!handled) {
    ...
    if (isVimBackInCommandMode &&
        rime_get_api_stdbool()->get_option(_session, "vim_mode") &&
        !rime_get_api_stdbool()->get_option(_session, "ascii_mode")) {
  ...
  if (handled) {
    ...
    if (is_chording_key &&
        rime_get_api_stdbool()->get_option(_session, "_chord_typing")) {
  ...
  @autoreleasepool {
    ...
    if (cursorPosition > targetPosition) {
      ...
      NSString* prefix = [[composition substringToIndex:cursorPosition]
          stringByReplacingOccurrencesOfString:@" "
                                    withString:@""];
      BOOL noneConverted = [_originalString
          hasSuffix:[[composition substringFromIndex:targetPosition]
                        stringByReplacingOccurrencesOfString:@" "
                                                  withString:@""]];
      while (targetPrefix.length < prefix.length) {
        BOOL byChar =
            noneConverted && ![[prefix substringFromIndex:targetPrefix.length]
                                 containsString:@" "];
        rime_get_api_stdbool()->process_key(
            _session,
            vertical ? (byChar ? XK_KP_Up : XK_Up)
                     : (byChar ? XK_KP_Left : XK_Left),
            0);
      ...
      while (targetSuffix.length < suffix.length) {
        rime_get_api_stdbool()->process_key(_session,
                                            vertical ? XK_Down : XK_Right, 0);
        rime_get_api_stdbool()->get_context(_session, &ctx);
...
- (void)onChordTimer:(NSTimer*)timer {
  ...
  if (_chordKeyCount > 0 && _session != 0) {
    ...
    for (int i = 0; i < _chordKeyCount; ++i) {
      if (rime_get_api_stdbool()->process_key(
              _session, _chordKeyCodes[i],
...
static NSString* getOptionLabel(RimeSessionId session,
                                const char* option,
                                bool state) {
  if (RimeStringSlice short_label =
          rime_get_api_stdbool()->get_state_label_abbreviated(session, option,
                                                              state, true);
      short_label.str != NULL &&
      short_label.length >= strlen(short_label.str)) {
    return @(short_label.str);
  } else {
    RimeStringSlice long_label =
        rime_get_api_stdbool()->get_state_label_abbreviated(session, option,
                                                            state, false);
...
- (void)activateServer:(id)sender {
  ...
  NSString* keyboardLayout =
      [NSApp.squirrelAppDelegate.config stringForOption:@"keyboard_layout"];
  if ([@"last" caseInsensitiveCompare:keyboardLayout] == NSOrderedSame ||
    ...
    keyboardLayout = [@"com.apple.keylayout." append:keyboardLayout];
  }
  ...
  if ([defaultConfig openWithConfigId:@"default"] &&
    ...
    _goodOldCapsLock =
        [defaultConfig boolValueForOption:@"ascii_composer/good_old_caps_lock"];
  }
  [defaultConfig close];
...
- (CGRect)getIbeamRect __attribute__((objc_direct)) {
  ...
  NSWidth(IbeamRect) > NSHeight(IbeamRect)
      ? IbeamRect.origin.x += _inlineOffset
      : IbeamRect.origin.y += _inlineOffset;
  if (@available(
          macOS 14.0, *)) {  // avoid overlapping with cursor effects view
    if ((_goodOldCapsLock &&
         (_lastModifiers & NSEventModifierFlagCapsLock) != 0) ||
...
- (void)rimeUpdate __attribute__((objc_direct)) {
  ...
  if (rime_get_api_stdbool()->get_status(_session, &status)) {
    ...
    if (_schemaId == nil ||
      ...
      _schemaId = @(status.schema_id);
      _showingSwitcherMenu =
          rime_get_api_stdbool()->get_option(_session, "dumb");
      if (!_showingSwitcherMenu) {
        ...
        rime_get_api_stdbool()->set_option(_session, "soft_cursor",
                                           !_inlinePreedit);
      } else {
  ...
  if (rime_get_api_stdbool()->get_context(_session, &ctx)) {
    ...
    NSString* originalString =
        @(rime_get_api_stdbool()->get_input(_session) ?: "");
    ...
    NSUInteger suffixLength = [[preeditText substringFromIndex:end]
                                  stringByReplacingOccurrencesOfString:@" "
                                                            withString:@""]
                                  .length;
    NSUInteger selLength =
        [[preeditText substringWithRange:NSMakeRange(start, end - start)]
            stringByReplacingOccurrencesOfString:@" "
                                      withString:@""]
            .length;
    ...
    NSRange selSegment = NSMakeRange(
        originalString.length - suffixLength - selLength, selLength);
    didCompose |= _selSegment.location != selSegment.location ||
                  (_selSegment.length != selSegment.length &&
    ...
    if (showingStatus) {
      ...
      if (_inlinePreedit) {
        if (end <= caretPos && caretPos < length) {
          candidatePreviewText = [candidatePreviewText
              append:[preeditText substringFromIndex:caretPos]];
        }
      ...
      if (_inlinePreedit) {
        if (_inlinePlaceholder && preeditText.length == 0 &&
            numCandidates > 0) {
    ...
    if (index < endIndex) {
      ...
      if (rime_get_api_stdbool()->candidate_list_from_index(_session, &iterator,
                                                            (int)index)) {
        while (index < endIndex &&
               rime_get_api_stdbool()->candidate_list_next(&iterator)) {
