...
- (CGFloat)annotateRubyInRange:(NSRange)range
  ...
  [regex enumerateMatchesInString:self.mutableString
    ...
    if ([self attributedSubstringFromRange:NSMakeRange(0, NSMaxRange(baseRange))].size.width > maxLength - 0.1) {
      ...
      CGFloat height = isVertical ? (baseFont.verticalFont.ascender - baseFont.verticalFont.descender)
...
- (NSAttributedString *)attributedStringHorizontalInVerticalForms {
  NSMutableDictionary *attrs = [[self attributesAtIndex:0 effectiveRange:NULL] mutableCopy];
  ...
  NSImage *image = [NSImage imageWithSize:NSMakeSize(height, width)
    ...
    CGContextTranslateCTM(context, NSWidth(dstRect) * 0.5, NSHeight(dstRect) * 0.5);
    CGContextRotateCTM(context, -M_PI_2);
    CGPoint origin = CGPointMake(- self.size.width / width * NSHeight(dstRect) * 0.5,
                                 - NSWidth(dstRect) * 0.5);
...
- (void)getLuminance:(CGFloat *)luminance
               ...
               bBuYl:(CGFloat *)bBuYl
               alpha:(CGFloat *)alpha {
  static CGFloat luminanceComponent, aGnRdComponent, bBuYlComponent, alphaComponent;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    CGFloat components[4] = {0.0, 0.0, 0.0, 1.0};
    [([self.colorSpace isEqualTo:NSColorSpace.labColorSpace] ? self :
       [self colorUsingColorSpace:NSColorSpace.labColorSpace]) getComponents:components];
  ...
  if (luminance != NULL) *luminance = luminanceComponent;
  if (aGnRd != NULL) *aGnRd = aGnRdComponent;
  if (bBuYl != NULL) *bBuYl = bBuYlComponent;
  if (alpha != NULL) *alpha = alphaComponent;
}
...
@property(nonatomic, strong, readonly, nonnull) NSDictionary *textAttrs;
...
@property(nonatomic, strong, readonly, nonnull) NSAttributedString *fullWidthPlaceholder;
...
- (instancetype)init {
  ...
  if (self) {
    ...
    NSFontDescriptor *userFontDesc = getFontDescriptor([NSFont userFontOfSize:0.0].fontName);
    NSFontDescriptor *monoFontDesc = getFontDescriptor([NSFont userFixedPitchFontOfSize:0.0].fontName);
...
- (void)updateSeperatorAndSymbolAttrs {
  ...
  _separator = [NSAttributedString.alloc initWithString:
                _linear ? (_tabular ? @"\u3000\t\x1D" : @"\u3000\x1D") : @"\n"
                                             attributes:sepAttrs];
  _fullWidthPlaceholder = [NSAttributedString.alloc initWithString:kFullWidthSpace
...
- (void)updateCandidateFormatForAttributesOnly:(BOOL)attrsOnly {
  ...
  if (!attrsOnly) {
    ...
    if ([[NSCharacterSet characterSetWithRange:NSMakeRange(0xFF10, 10)]
      ...
      if ((enumRange = [candidateFormat rangeOfString:@"%c\u20E3"
        ...
        for (NSUInteger i = 0; i < labels.count; ++i) {
          labels[i] = [NSString stringWithFormat:@"%S",
                       (const unichar[3]){[labels[i] characterAtIndex:0] - 0xFF10 + 0x0030, 0xFE0E, 0x20E3}];
        }
      } else if ((enumRange = [candidateFormat rangeOfString:@"%c\u20DD"
                               options:NSLiteralSearch]).length > 0) {  // ①..⑨⓪
        for (NSUInteger i = 0; i < labels.count; ++i) {
          labels[i] = [NSString stringWithFormat:@"%S",
                       (const unichar[1]){[labels[i] characterAtIndex:0] == 0xFF10 ? 0x24EA :
        ...
        for (NSUInteger i = 0; i < labels.count; ++i) {
          labels[i] = [NSString stringWithFormat:@"%S",
                       (const unichar[1]){[labels[i] characterAtIndex:0] == 0xFF10 ? 0x247D :
        ...
        for (NSUInteger i = 0; i < labels.count; ++i) {
          labels[i] = [NSString stringWithFormat:@"%S",
                       (const unichar[2]){[labels[i] characterAtIndex:0] == 0xFF10 ? 0xD83C :
            [labels[i] characterAtIndex:0] - 0xFF11 + 0x2488,
            [labels[i] characterAtIndex:0] == 0xFF10 ? 0xDD00 : 0x0}];
        }
      } else if ((enumRange = [candidateFormat rangeOfString:@"%c,"
                               options:NSLiteralSearch]).length > 0) {  // 🄂..🄊🄁
        for (NSUInteger i = 0; i < labels.count; ++i) {
          labels[i] = [NSString stringWithFormat:@"%S",
                       (const unichar[2]){0xD83C, [labels[i] characterAtIndex:0] - 0xFF10 + 0xDD01}];
        }
      }
      ...
      if ((enumRange = [candidateFormat rangeOfString:@"%c\u20DD"
        ...
        for (NSUInteger i = 0; i < labels.count; ++i) {
          labels[i] = [NSString stringWithFormat:@"%S",
                       (const unichar[1]){[labels[i] characterAtIndex:0] - 0xFF21 + 0x24B6}];
        }
      } else if ((enumRange = [candidateFormat rangeOfString:@"(%c)"
                               options:NSLiteralSearch]).length > 0) {  // 🄐..🄩
        for (NSUInteger i = 0; i < labels.count; ++i) {
          labels[i] = [NSString stringWithFormat:@"%S",
                       (const unichar[2]){0xD83C, [labels[i] characterAtIndex:0] - 0xFF21 + 0xDD10}];
        }
      } else if ((enumRange = [candidateFormat rangeOfString:@"%c\u20DE"
                               options:NSLiteralSearch]).length > 0) {  // 🄰..🅉
        for (NSUInteger i = 0; i < labels.count; ++i) {
          labels[i] = [NSString stringWithFormat:@"%S",
                       (const unichar[2]){0xD83C, [labels[i] characterAtIndex:0] - 0xFF21 + 0xDD30}];
        }
      }
    }
    ...
    candTemplate = [NSMutableAttributedString.alloc initWithString:candidateFormat];
  } else {
  ...
  NSRange textRange = [candTemplate.mutableString rangeOfString:@"%@" options:NSLiteralSearch];
  ...
  if (!attrsOnly) {
    ...
    textRange = [candTemplate.mutableString rangeOfString:@"%@" options:NSLiteralSearch];
    ...
    if (commentRange.length > 0) {
      [candTemplate replaceCharactersInRange:commentRange
                    withString:[kTipSpecifier stringByAppendingString:
                                [candTemplate.mutableString substringWithRange:commentRange]]];
    } else {
      [candTemplate appendAttributedString:
  ...
  if (!_linear) {
    CGFloat indent = 0.0;
    NSAttributedString *labelFormat = [candTemplate attributedSubstringFromRange:
    ...
    for (NSString *label in _labels) {
      ...
      [enumString.mutableString replaceOccurrencesOfString:@"%c"
                                                withString:label
...
static double inline positive(double param) { return param > 0.0 ? param : 0.0; }
static double inline pos_round(double param) { return param > 0.0 ? round(param) : 0.0; }
static double inline pos_ceil(double param) { return param > 0.0 ? ceil(param) : 0.0; }
static double inline clamp_uni(double param) { return param > 0.0 ? (param < 1.0 ? param : 1.0) : 0.0; }
...
- (void)updateWithConfig:(SquirrelConfig *)config
  ...
  NSNumber *fontSize = [config getOptionalDoubleForOption:@"style/font_point" applyConstraint:pos_round];
  NSString *labelFontName = [config getStringForOption:@"style/label_font_face"];
  NSNumber *labelFontSize = [config getOptionalDoubleForOption:@"style/label_font_point" applyConstraint:pos_round];
  NSString *commentFontName = [config getStringForOption:@"style/comment_font_face"];
  NSNumber *commentFontSize = [config getOptionalDoubleForOption:@"style/comment_font_point" applyConstraint:pos_round];
  NSNumber *opacity = [config getOptionalDoubleForOption:@"style/opacity" alias:@"alpha" applyConstraint:clamp_uni];
  NSNumber *translucency = [config getOptionalDoubleForOption:@"style/translucency" applyConstraint:clamp_uni];
  NSNumber *cornerRadius = [config getOptionalDoubleForOption:@"style/corner_radius" applyConstraint:positive];
  NSNumber *hilitedCornerRadius = [config getOptionalDoubleForOption:@"style/hilited_corner_radius" applyConstraint:positive];
  NSNumber *borderHeight = [config getOptionalDoubleForOption:@"style/border_height" applyConstraint:pos_ceil];
  NSNumber *borderWidth = [config getOptionalDoubleForOption:@"style/border_width" applyConstraint:pos_ceil];
  NSNumber *lineSpacing = [config getOptionalDoubleForOption:@"style/line_spacing" applyConstraint:pos_round];
  ...
  if (appear == darkAppear) {
    for (NSString *option in styleOptions) {
      if ((colorScheme = [config getStringForOption:
                          [NSString stringWithFormat:@"style/%@/color_scheme_dark", option]])) {
        break;
      }
    }
  ...
  if (!colorScheme) {
    for (NSString *option in styleOptions) {
      if ((colorScheme = [config getStringForOption:
                          [NSString stringWithFormat:@"style/%@/color_scheme", option]])) {
        break;
      }
    }
  ...
  for (NSString *prefix in configPrefixes) {
    ...
    hilitedPreeditBackColor = [config getColorForOption:[prefix stringByAppendingString:
                                                         @"/hilited_back_color"]] ? : hilitedPreeditBackColor;
    hilitedPreeditForeColor = [config getColorForOption:[prefix stringByAppendingString:
                                                         @"/hilited_text_color"]] ? : hilitedPreeditForeColor;
    hilitedCandidateBackColor = [config getColorForOption:[prefix stringByAppendingString:
                                                           @"/hilited_candidate_back_color"]] ? : hilitedCandidateBackColor;
    hilitedTextForeColor = [config getColorForOption:[prefix stringByAppendingString:
                                                      @"/hilited_candidate_text_color"]] ? : hilitedTextForeColor;
    hilitedCommentForeColor = [config getColorForOption:[prefix stringByAppendingString:
                                                         @"/hilited_comment_text_color"]] ? : hilitedCommentForeColor;
    // for backward compatibility, 'label_hilited_color' and 'hilited_candidate_label_color' are both valid
    hilitedLabelForeColor = [config getColorForOption:[prefix stringByAppendingString:@"/label_hilited_color"]
    ...
    opacity = [config getOptionalDoubleForOption:[prefix stringByAppendingString:@"/opacity"]
                                           alias:@"alpha" applyConstraint:clamp_uni] ? : opacity;
    ...
    hilitedCornerRadius = [config getOptionalDoubleForOption:[prefix stringByAppendingString:@"/hilited_corner_radius"]
                                             applyConstraint:positive] ? : hilitedCornerRadius;
    ...
    baseOffset = [config getOptionalDoubleForOption:[prefix stringByAppendingString:
  ...
  NSMutableParagraphStyle *candidateParagraphStyle = _candidateParagraphStyle.mutableCopy;
  candidateParagraphStyle.alignment = linear ? NSTextAlignmentNatural : NSTextAlignmentLeft;
  ...
  NSMutableDictionary *labelAttrs = _labelAttrs.mutableCopy;
  NSMutableDictionary *commentAttrs = _commentAttrs.mutableCopy;
  NSMutableDictionary *preeditAttrs = _preeditAttrs.mutableCopy;
  NSMutableDictionary *pagingAttrs = _pagingAttrs.mutableCopy;
  NSMutableDictionary *statusAttrs = _statusAttrs.mutableCopy;
  ...
  NSDictionary *baselineRefInfo =
    @{(id)kCTBaselineReferenceFont : vertical ? refFont.verticalFont : refFont,
      (id)kCTBaselineClassIdeographicCentered : @(vertical ? 0.0 : refFont.ascender * 0.5 + refFont.descender * 0.5),
      (id)kCTBaselineClassRoman : @(vertical ? - refFont.verticalFont.ascender * 0.5 - refFont.verticalFont.descender * 0.5 : 0.0),
      (id)kCTBaselineClassIdeographicLow : @(vertical ? refFont.verticalFont.descender * 0.5 - refFont.verticalFont.ascender * 0.5 : refFont.descender)};
  ...
  if (@available(macOS 10.14, *)) {
    if (translucency.doubleValue > 0.001 && !isNative && backColor != nil &&
      ...
      backColor = [backColor colorByInvertingLuminanceToExtent:kDefaultColorInversion];
      borderColor = [borderColor colorByInvertingLuminanceToExtent:kDefaultColorInversion];
      preeditBackColor = [preeditBackColor colorByInvertingLuminanceToExtent:kDefaultColorInversion];
      preeditForeColor = [preeditForeColor colorByInvertingLuminanceToExtent:kDefaultColorInversion];
      textForeColor = [textForeColor colorByInvertingLuminanceToExtent:kDefaultColorInversion];
      commentForeColor = [commentForeColor colorByInvertingLuminanceToExtent:kDefaultColorInversion];
      labelForeColor = [labelForeColor colorByInvertingLuminanceToExtent:kDefaultColorInversion];
  ...
  labelForeColor = labelForeColor ? : isNative ? NSColor.accentColor : blendColors(textForeColor, backColor);
...
- (void)setAnnotationHeight:(CGFloat)height {
  if (height > 0.1 && _linespace < height * 2) {
    ...
    [candTemplate addAttribute:NSParagraphStyleAttributeName
                         ...
                         range:NSMakeRange(0, candTemplate.length)];
    _candidateTemplate = candTemplate;
    NSMutableAttributedString *candHilitedTemplate = _candidateHilitedTemplate.mutableCopy;
    [candHilitedTemplate addAttribute:NSParagraphStyleAttributeName
                                value:candidateParagraphStyle
                                range:NSMakeRange(0, candHilitedTemplate.length)];
    _candidateHilitedTemplate = candHilitedTemplate;
    if (_tabular) {
      NSMutableAttributedString *candDimmedTemplate = _candidateDimmedTemplate.mutableCopy;
      [candDimmedTemplate addAttribute:NSParagraphStyleAttributeName
                                 value:candidateParagraphStyle
...
- (void)setScriptVariant:(NSString *)scriptVariant {
  ...
  statusAttrs[(id)kCTBaselineReferenceInfoAttributeName] =
    @{(id)kCTBaselineReferenceFont : _vertical ? zhCommentFont.verticalFont : zhCommentFont};
...
- (void)drawGlyphsForGlyphRange:(NSRange)glyphsToShow
  ...
  [self.textStorage
   ...
   usingBlock:^(NSDictionary<NSAttributedStringKey, id> * _Nonnull attrs,
    ...
    NSRect lineRect = [self lineFragmentRectForGlyphAtIndex:glyphRange.location
                                             effectiveRange:NULL
                                    withoutAdditionalLayout:YES];
    CGContextSaveGState(context);
    if (attrs[(id)kCTRubyAnnotationAttributeName]) {
      ...
      NSPoint position = [self locationForGlyphAtIndex:glyphRange.location];
...
- (BOOL)                        layoutManager:(NSLayoutManager *)layoutManager
  ...
  if (charIndex <= 1) {
    ...
    NSTextAlignment alignment = [[layoutManager.textStorage attribute:NSParagraphStyleAttributeName
                                                              atIndex:charIndex
...
- (void)drawAtPoint:(CGPoint)point
  ...
  for (NSTextLineFragment *lineFrag in self.textLineFragments) {
    ...
    CGFloat lineSpacing = [[lineFrag.attributedString attribute:NSParagraphStyleAttributeName
...
- (BOOL)      textLayoutManager:(NSTextLayoutManager *)textLayoutManager
  ...
  if (charIndex <= 1) {
    ...
    unichar charBeforeIndex = [contentStorage.textStorage.mutableString
                               characterAtIndex:charIndex - 1];
    NSTextAlignment alignment = [[contentStorage.textStorage attribute:NSParagraphStyleAttributeName
                                                               atIndex:charIndex
                                                        effectiveRange:NULL] alignment];
    if (alignment == NSTextAlignmentNatural) {  // candidates in linear layout
      return charBeforeIndex == 0x1D;
...
- (NSTextLayoutFragment *)textLayoutManager:(NSTextLayoutManager *)textLayoutManager
  ...
  SquirrelTextLayoutFragment *fragment = [SquirrelTextLayoutFragment.alloc
                                          initWithTextElement:textElement range:textRange];
  NSTextStorage *textStorage = textLayoutManager.textContainer.textView.textContentStorage.textStorage;
  if (textStorage.length > 0 && [location isEqual:self.documentRange.location]) {
    fragment.topMargin = [[textStorage attribute:NSParagraphStyleAttributeName
                                         atIndex:0 effectiveRange:NULL] lineSpacing];
  }
  return fragment;
}
...
static SquirrelTheme *_darkTheme API_AVAILABLE(macos(10.14)) = SquirrelTheme.alloc.init;
...
- (void)layoutContents {
  if (@available(macOS 12.0, *)) {
    [_textView.textLayoutManager ensureLayoutForRange:
     _textView.textContentStorage.documentRange];
...
- (NSRect)blockRectForRange:(NSRange)charRange {
  ...
  if (@available(macOS 12.0, *)) {
    ...
    NSRect __block blockRect = CGRectNull;
    ...
    if (_currentTheme.linear && _currentTheme.linespace > 0.1 && _numCandidates > 0) {
      if (charRange.location >= _candidateRanges[0].location &&
          charRange.location < NSMaxRange(_candidateRanges[_numCandidates - 1])) {
        CGFloat lineSpaceBefore = ceil(_currentTheme.linespace * 0.5);
        blockRect.size.height += lineSpaceBefore;
        blockRect.origin.y -= lineSpaceBefore;
      }
      if (NSMaxRange(charRange) > _candidateRanges[0].location &&
          NSMaxRange(charRange) <= NSMaxRange(_candidateRanges[_numCandidates - 1])) {
        CGFloat lineSpaceAfter = floor(_currentTheme.linespace * 0.5);
        blockRect.size.height += lineSpaceAfter;
      }
    }
    return blockRect;
  } else {
...
- (SquirrelTextPolygon)textPolygonForRange:(NSRange)charRange {
  SquirrelTextPolygon textPolygon = {NSZeroRect, NSZeroRect, NSZeroRect};
  if (charRange.location == NSNotFound) {
    return textPolygon;
  }
  if (@available(macOS 12.0, *)) {
     ...
     usingBlock:^BOOL(NSTextRange * _Nullable segRange, CGRect segFrame,
      ...
      if (!CGRectIsEmpty(segFrame)) {
        if (NSIsEmptyRect(leadingLineRect) || CGRectGetMinY(segFrame) < NSMaxY(leadingLineRect)) {
          ...
          leadingLineRange = [leadingLineRange textRangeByFormingUnionWithTextRange:segRange];
        } else {
          trailingLineRect = NSUnionRect(segFrame, trailingLineRect);
          trailingLineRange = [trailingLineRange textRangeByFormingUnionWithTextRange:segRange];
        }
      }
      return YES;
    }];
    ...
    if (NSIsEmptyRect(trailingLineRect)) {
      textPolygon.bodyRect = leadingLineRect;
    } else {
      if (_currentTheme.linear && _currentTheme.linespace > 0.1 &&_numCandidates > 0) {
        if (NSMaxRange(charRange) > _candidateRanges[0].location &&
            NSMaxRange(charRange) <= NSMaxRange(_candidateRanges[_numCandidates - 1])) {
      ...
      if (fabs(NSMaxX(trailingLineRect) - NSMaxX(leadingLineRect)) < 1) {
        if (fabs(NSMinX(leadingLineRect) - NSMinX(trailingLineRect)) < 1) {
          textPolygon.bodyRect = NSUnionRect(leadingLineRect, trailingLineRect);
        } else {
          textPolygon.leadingRect = leadingLineRect;
          textPolygon.bodyRect = NSMakeRect(0.0, NSMaxY(leadingLineRect), containerWidth,
                                            NSMaxY(trailingLineRect) - NSMaxY(leadingLineRect));
        }
      } else {
        textPolygon.trailingRect = trailingLineRect;
        if (fabs(NSMinX(leadingLineRect) - NSMinX(trailingLineRect)) < 1) {
          textPolygon.bodyRect = NSMakeRect(0.0, NSMinY(leadingLineRect), containerWidth,
                                            NSMinY(trailingLineRect) - NSMinY(leadingLineRect));
        } else {
          textPolygon.leadingRect = leadingLineRect;
          if (![trailingLineRange containsLocation:leadingLineRange.endLocation]) {
            textPolygon.bodyRect = NSMakeRect(0.0, NSMaxY(leadingLineRect), containerWidth,
    ...
    if (NSMaxRange(leadingLineRange) >= NSMaxRange(glyphRange)) {
      ...
      textPolygon.bodyRect = NSMakeRect(headX, NSMinY(leadingLineRect),
      ...
      if (NSMaxRange(trailingLineRange) == NSMaxRange(glyphRange)) {
        if (glyphRange.location == leadingLineRange.location) {
          ...
          textPolygon.leadingRect = NSMakeRect(headX, NSMinY(leadingLineRect),
                                               containerWidth - headX, NSHeight(leadingLineRect));
          textPolygon.bodyRect = NSMakeRect(0.0, NSMaxY(leadingLineRect), containerWidth,
                                            NSMaxY(trailingLineRect) - NSMaxY(leadingLineRect));
        }
      } else {
        textPolygon.trailingRect = NSMakeRect(0.0, NSMinY(trailingLineRect),
...
- (void)estimateBoundsForPreedit:(NSRange)preeditRange
  ...
  if (_currentTheme.linear) {
    ...
    if (numCandidates > 0) {
      BOOL truncated = _truncated[0];
      NSUInteger start = _candidateRanges[0].location;
      for (NSUInteger i = 1; i <= numCandidates; truncated = _truncated[i++]) {
        if (i == numCandidates || _truncated[i] != truncated) {
          NSUInteger end = NSMaxRange(_candidateRanges[i - 1]);
          NSRect candidateRect = [self blockRectForRange:NSMakeRange(start, end - start)];
          width = fmax(width, ceil(NSMaxX(candidateRect)) - _currentTheme.fullWidth * !truncated);
          start = end;
        }
      }
    }
...
- (void)drawViewWithInsets:(NSEdgeInsets)alignmentRectInsets
  ...
  if (_currentTheme.linear && numCandidates == 0 && preeditRange.length == 0) {
    _trailPadding = 0.0;
  }
  ...
  _textView.needsDisplayInRect = [self convertRect:self.bounds toView:_textView];
  [self layoutContents];
}
...
- (void)setPreeditRange:(NSRange)preeditRange
  ...
  _textView.needsDisplayInRect = [self convertRect:_preeditBlock toView:_textView];
  [self layoutContents];
}
...
static NSBezierPath *squirclePath(NSPointArray vertices,
  ...
  for (NSInteger i = 0; i < numVert; ++i) {
    ...
    if (fabs(nextDiff.dx) >= fabs(nextDiff.dy)) {
      arcRadius = fmin(radius, fmin(fabs(nextDiff.dx), fabs(lastDiff.dy)) * 0.5);
      point.y = nextPoint.y;
      ...
      arcRadius = fmin(radius, fmin(fabs(nextDiff.dy), fabs(lastDiff.dx)) * 0.5);
      point.x = nextPoint.x;
...
static void rectVertices(NSRect rect,
...
static void textPolygonVertices(SquirrelTextPolygon textPolygon,
                                NSPointArray vertices) {
  switch ((NSIsEmptyRect(textPolygon.leadingRect) << 2) |
          (NSIsEmptyRect(textPolygon.bodyRect) << 1) |
          (NSIsEmptyRect(textPolygon.trailingRect) << 0)) {
    case 0b011:
      rectVertices(textPolygon.leadingRect, vertices);
      break;
    case 0b110:
      rectVertices(textPolygon.trailingRect, vertices);
      break;
    case 0b101:
      rectVertices(textPolygon.bodyRect, vertices);
      break;
    case 0b001: {
      NSPoint leadingVertices[4], bodyVertices[4];
      rectVertices(textPolygon.leadingRect, leadingVertices);
      rectVertices(textPolygon.bodyRect, bodyVertices);
    ...
    case 0b100: {
      ...
      rectVertices(textPolygon.bodyRect, bodyVertices);
      rectVertices(textPolygon.trailingRect, trailingVertices);
    ...
    case 0b010:
      if (NSMinX(textPolygon.leadingRect) <= NSMaxX(textPolygon.trailingRect)) {
...
- (CAShapeLayer *)getFunctionButtonLayer {
  ...
  if (!NSIsEmptyRect(buttonRect) && buttonColor) {
    CGFloat cornerRadius = fmin(_currentTheme.hilitedCornerRadius, NSHeight(buttonRect) * 0.5);
...
- (void)updateLayer {
  ...
  NSRect backgroundRect = NSInsetRect(panelRect, theme.borderInsets.width, theme.borderInsets.height);
  ...
  if (_numCandidates > 0) {
    candidateBlockRange = NSUnionRange(_candidateRanges[0], _candidateRanges[_numCandidates - 1]);
  ...
  if (preeditRange.length > 0) {
    ...
    NSRange hilitedPreeditRange = NSIntersectionRange(_hilitedPreeditRange, visibleRange);
    CGFloat cornerRadius = fmin(theme.hilitedCornerRadius,
    ...
    if (hilitedPreeditRange.length > 0 && theme.hilitedPreeditBackColor) {
      ...
      innerBox.origin.x += _alignmentRectInsets.left - padding;
      innerBox.size.width = backgroundRect.size.width - theme.fullWidth + padding * 2;
      innerBox.origin.y += _alignmentRectInsets.top;
      innerBox = [self backingAlignedRect:innerBox
                                  options:NSAlignAllEdgesNearest];
      SquirrelTextPolygon textPolygon = [self textPolygonForRange:hilitedPreeditRange];
      NSInteger numVert = 0;
      if (!NSIsEmptyRect(textPolygon.leadingRect)) {
        textPolygon.leadingRect.origin.x += _alignmentRectInsets.left - padding;
        textPolygon.leadingRect.origin.y += _alignmentRectInsets.top;
        textPolygon.leadingRect.size.width += padding * 2;
        textPolygon.leadingRect = [self backingAlignedRect:NSIntersectionRect(textPolygon.leadingRect, innerBox)
                                                   options:NSAlignAllEdgesNearest];
        numVert += 4;
      }
      if (!NSIsEmptyRect(textPolygon.bodyRect)) {
        textPolygon.bodyRect.origin.x += _alignmentRectInsets.left - padding;
        textPolygon.bodyRect.origin.y += _alignmentRectInsets.top;
        textPolygon.bodyRect.size.width += padding;
        if (!NSIsEmptyRect(textPolygon.trailingRect) ||
            NSMaxRange(hilitedPreeditRange) + 2 == NSMaxRange(preeditRange)) {
          textPolygon.bodyRect.size.width += padding;
        }
        textPolygon.bodyRect = [self backingAlignedRect:NSIntersectionRect(textPolygon.bodyRect, innerBox)
                                                options:NSAlignAllEdgesNearest];
        numVert += 2;
      }
      if (!NSIsEmptyRect(textPolygon.trailingRect)) {
        textPolygon.trailingRect.origin.x += _alignmentRectInsets.left - padding;
        textPolygon.trailingRect.origin.y += _alignmentRectInsets.top;
        textPolygon.trailingRect.size.width += padding;
        if (NSMaxRange(hilitedPreeditRange) + 2 == NSMaxRange(preeditRange)) {
          textPolygon.trailingRect.size.width += padding;
        }
        textPolygon.trailingRect = [self backingAlignedRect:NSIntersectionRect(textPolygon.trailingRect, innerBox)
                                                    options:NSAlignAllEdgesNearest];
        numVert += 4;
      }

      // Handles the special case where containing boxes are separated
      if (NSIsEmptyRect(textPolygon.bodyRect) && !NSIsEmptyRect(textPolygon.leadingRect) &&
          !NSIsEmptyRect(textPolygon.trailingRect) &&
          NSMaxX(textPolygon.trailingRect) < NSMinX(textPolygon.leadingRect)) {
        NSPoint leadingVertices[4], trailingVertices[4];
        rectVertices(textPolygon.leadingRect, leadingVertices);
        rectVertices(textPolygon.trailingRect, trailingVertices);
        hilitedPreeditPath = squirclePath(leadingVertices, 4, cornerRadius);
        [hilitedPreeditPath appendBezierPath:squirclePath(trailingVertices, 4, cornerRadius)];
      } else {
        numVert = numVert > 8 ? 8 : numVert < 4 ? 4 : numVert;
        NSPoint polygonVertices[numVert];
        textPolygonVertices(textPolygon, polygonVertices);
        hilitedPreeditPath = squirclePath(polygonVertices, numVert, cornerRadius);
      }
    }
    _deleteBackRect = [self blockRectForRange:NSMakeRange(NSMaxRange(preeditRange) - 1, 1)];
    ...
    _deleteBackRect = [self backingAlignedRect:NSIntersectionRect(_deleteBackRect, _preeditBlock)
                                       options:NSAlignAllEdgesNearest];
  }
  ...
  if (candidateBlockRange.length > 0) {
    ...
    CGFloat blockCornerRadius = fmin(theme.hilitedCornerRadius, NSHeight(_candidateBlock) * 0.5);
    candidateBlockPath = squirclePath(candidateBlockVertices, 4, blockCornerRadius);

    // Draw candidate highlight rect
    CGFloat cornerRadius = fmin(theme.hilitedCornerRadius,
                                theme.candidateParagraphStyle.minimumLineHeight * 0.5);
    _candidatePolygons = new SquirrelTextPolygon[_numCandidates];
    if (theme.linear) {
      CGFloat gridOriginY;
      CGFloat tabInterval;
      ...
      for (NSUInteger i = 0; i < _numCandidates; ++i) {
        NSRange candidateRange = NSIntersectionRange(_candidateRanges[i], visibleRange);
        if (candidateRange.length == 0) {
          _numCandidates = i;
          break;
        }
        SquirrelTextPolygon candidatePolygon = [self textPolygonForRange:candidateRange];
        if (!NSIsEmptyRect(candidatePolygon.leadingRect)) {
          candidatePolygon.leadingRect.origin.x += theme.borderInsets.width;
          candidatePolygon.leadingRect.size.width += theme.fullWidth;
          candidatePolygon.leadingRect.origin.y += _alignmentRectInsets.top;
          candidatePolygon.leadingRect = [self backingAlignedRect:NSIntersectionRect(candidatePolygon.leadingRect, _candidateBlock)
                                                          options:NSAlignAllEdgesNearest];
        }
        if (!NSIsEmptyRect(candidatePolygon.trailingRect)) {
          candidatePolygon.trailingRect.origin.x += theme.borderInsets.width;
          candidatePolygon.trailingRect.origin.y += _alignmentRectInsets.top;
          candidatePolygon.trailingRect = [self backingAlignedRect:NSIntersectionRect(candidatePolygon.trailingRect, _candidateBlock)
                                                           options:NSAlignAllEdgesNearest];
        }
        if (!NSIsEmptyRect(candidatePolygon.bodyRect)) {
          candidatePolygon.bodyRect.origin.x += theme.borderInsets.width;
          if (_truncated[i]) {
            candidatePolygon.bodyRect.size.width = NSMaxX(_candidateBlock) - NSMinX(candidatePolygon.bodyRect);
          } else if (!NSIsEmptyRect(candidatePolygon.trailingRect)) {
            candidatePolygon.bodyRect.size.width += theme.fullWidth;
          }
          candidatePolygon.bodyRect.origin.y += _alignmentRectInsets.top;
          candidatePolygon.bodyRect = [self backingAlignedRect:NSIntersectionRect(candidatePolygon.bodyRect, _candidateBlock)
                                                       options:NSAlignAllEdgesNearest];
        }
        if (theme.tabular) {
          if (_expanded) {
            if (i % theme.pageSize == 0) {
              ...
              sectionRect.size.height = NSMaxY(NSIsEmptyRect(candidatePolygon.trailingRect) ? candidatePolygon.bodyRect :
                                               candidatePolygon.trailingRect) - NSMinY(sectionRect);
              ...
              if (sec == _hilitedIndex / theme.pageSize) {
                ...
                CGFloat pageCornerRadius = fmin(theme.hilitedCornerRadius, NSHeight(sectionRect) * 0.5);
                activePagePath = squirclePath(activePageVertices, 4, pageCornerRadius);
              }
            }
          }
          CGFloat bottomEdge = NSMaxY(NSIsEmptyRect(candidatePolygon.trailingRect) ?
                                      candidatePolygon.bodyRect : candidatePolygon.trailingRect);
          if (fabs(bottomEdge - gridOriginY) > 2) {
             lineNum += i > 0 ? 1 : 0;
            // horizontal border except for the last line
            if (fabs(bottomEdge - NSMaxY(_candidateBlock)) > 2) {
              [gridPath moveToPoint:NSMakePoint(NSMinX(_candidateBlock) + ceil(theme.fullWidth * 0.5), bottomEdge)];
              [gridPath lineToPoint:NSMakePoint(NSMaxX(_candidateBlock) - floor(theme.fullWidth * 0.5), bottomEdge)];
            }
            gridOriginY = bottomEdge;
          }
          NSPoint headOrigin = (NSIsEmptyRect(candidatePolygon.leadingRect) ?
                                candidatePolygon.bodyRect : candidatePolygon.leadingRect).origin;
          NSUInteger headTabColumn = (NSUInteger)round((headOrigin.x - _alignmentRectInsets.left) / tabInterval);
          // vertical bar
          if (headOrigin.x > NSMinX(_candidateBlock) + theme.fullWidth) {
            ...
            [gridPath lineToPoint:NSMakePoint(headOrigin.x, NSMaxY(NSIsEmptyRect(candidatePolygon.leadingRect) ?
                                  candidatePolygon.bodyRect : candidatePolygon.leadingRect) - cornerRadius * 0.8)];
          }
          _tabularIndices[i] = (SquirrelTabularIndex){i, lineNum, headTabColumn};
        }
        _candidatePolygons[i] = candidatePolygon;
      }
      if (_hilitedIndex < _numCandidates) {
        NSInteger numVert = (NSIsEmptyRect(_candidatePolygons[_hilitedIndex].leadingRect) ? 0 : 4) +
        (NSIsEmptyRect(_candidatePolygons[_hilitedIndex].bodyRect) ? 0 : 2) +
        (NSIsEmptyRect(_candidatePolygons[_hilitedIndex].trailingRect) ? 0 : 4);
        // Handles the special case where containing boxes are separated
        if (numVert == 8 && NSMaxX(_candidatePolygons[_hilitedIndex].trailingRect) <
                            NSMinX(_candidatePolygons[_hilitedIndex].leadingRect)) {
          NSPoint leadingVertices[4], trailingVertices[4];
          rectVertices(_candidatePolygons[_hilitedIndex].leadingRect, leadingVertices);
          rectVertices(_candidatePolygons[_hilitedIndex].trailingRect, trailingVertices);
          hilitedCandidatePath = squirclePath(leadingVertices, 4, cornerRadius);
          [hilitedCandidatePath appendBezierPath:squirclePath(trailingVertices, 4, cornerRadius)];
        } else {
          numVert = numVert > 8 ? 8 : numVert < 4 ? 4 : numVert;
          NSPoint polygonVertices[numVert];
          textPolygonVertices(_candidatePolygons[_hilitedIndex], polygonVertices);
          hilitedCandidatePath = squirclePath(polygonVertices, numVert, cornerRadius);
        }
      }
    } else {  // stacked layout
      for (NSUInteger i = 0; i < _numCandidates; ++i) {
      ...
      if (_hilitedIndex < _numCandidates) {
        ...
        rectVertices(_candidatePolygons[_hilitedIndex].bodyRect, candidateVertices);
  ...
  if (pagingRange.length > 0) {
    ...
    if (theme.showPaging) {
      ...
      _pageDownRect = [self blockRectForRange:NSMakeRange(NSMaxRange(pagingRange) - 1, 1)];
      ...
      _pageUpRect = [self backingAlignedRect:NSIntersectionRect(_pageUpRect, _pagingBlock)
                                     options:NSAlignAllEdgesNearest];
      _pageDownRect = [self backingAlignedRect:NSIntersectionRect(_pageDownRect, _pagingBlock)
                                       options:NSAlignAllEdgesNearest];
    }
    if (theme.tabular) {
      _expanderRect = [self blockRectForRange:NSMakeRange(pagingRange.location + pagingRange.length / 2, 1)];
      _expanderRect.origin.x += theme.borderInsets.width;
      _expanderRect.size.width += theme.fullWidth;
      _expanderRect.origin.y += _alignmentRectInsets.top;
      _expanderRect = [self backingAlignedRect:NSIntersectionRect(_expanderRect, backgroundRect)
                                       options:NSAlignAllEdgesNearest];
    }
  }
  ...
  CGFloat innerCornerRadius = fmax(fmin(theme.hilitedCornerRadius, NSHeight(backgroundRect) * 0.5),
  ...
  if (!theme.linear || pagingRange.length == 0) {
    ...
    NSRect tailPanelRect = NSInsetRect(NSOffsetRect(_pagingBlock, 0, theme.borderInsets.height), -theme.borderInsets.width, 0);
    textPolygonVertices((SquirrelTextPolygon){mainPanelRect, tailPanelRect, NSZeroRect}, panelVertices);
    ...
    textPolygonVertices((SquirrelTextPolygon){mainBackgroundRect, _pagingBlock, NSZeroRect}, backgroundVertices);
  ...
  if ((!NSIsEmptyRect(_preeditBlock) || !NSIsEmptyRect(_pagingBlock) ||
       !NSIsEmptyRect(_expanderRect)) && theme.preeditBackColor) {
  ...
  if (hilitedCandidatePath && theme.hilitedCandidateBackColor) {
    if (activePagePath) {
      ...
      activePageLayer.fillColor = [[theme.hilitedCandidateBackColor
  ...
  if (gridPath) {
    ...
    gridLayer.strokeColor = [theme.commentForeColor
                             blendedColorWithFraction:0.8
...
- (SquirrelIndex)getIndexFromMouseSpot:(NSPoint)spot {
  ...
  if (NSMouseInRect(point, self.bounds, YES)) {
    if (NSMouseInRect(point, _preeditBlock, YES)) {
      return NSMouseInRect(point, _deleteBackRect, YES) ? kBackSpaceKey : kCodeInputArea;
    }
    ...
    for (NSUInteger i = 0; i < _numCandidates; ++i) {
      if (NSMouseInRect(point, _candidatePolygons[i].bodyRect, YES) ||
          NSMouseInRect(point, _candidatePolygons[i].leadingRect, YES) ||
          NSMouseInRect(point, _candidatePolygons[i].trailingRect, YES)) {
        return i;
      }
    }
  }
...
@property(nonatomic, strong, readonly, nullable) NSTimer *displayTimer;
@property(nonatomic, strong, readonly, nullable) NSTimer *hideTimer;

- (void)showWithToolTip:(NSString * _Nullable)toolTip
              withDelay:(BOOL)delay;
...
- (void)showWithToolTip:(NSString *)toolTip
  ...
  if (delay) {
    _displayTimer = [NSTimer scheduledTimerWithTimeInterval:3.0
                                                     target:self
...
@implementation SquirrelPanel {
  ...
  NSMutableArray<NSString *> *_candTexts;
  NSMutableArray<NSString *> *_candComments;
  NSRange _indexRange;
...
- (BOOL)firstLine {
  return _view.tabularIndices ? _view.tabularIndices[_highlightedIndex].lineNum == 0 : YES;
}
  ...
  if ([object isKindOfClass:SquirrelInputController.class] &&
    ...
    if (@available(macOS 10.14, *)) {
      ...
      NSAppearanceName appearName = [clientAppearance bestMatchFromAppearancesWithNames:
                                     @[NSAppearanceNameAqua, NSAppearanceNameDarkAqua]];
      SquirrelAppear appear = [appearName isEqualToString:
                               NSAppearanceNameDarkAqua] ? darkAppear : defaultAppear;
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _candTexts = NSMutableArray.alloc.init;
    _candComments = NSMutableArray.alloc.init;
...
- (NSUInteger)candidateIndexOnDirection:(SquirrelIndex)arrowKey {
  if (!self.tabular || _indexRange.length == 0 || _highlightedIndex == NSNotFound) {
    return NSNotFound;
  }
...
- (void)sendEvent:(NSEvent *)event {
  ...
  switch (event.type) {
        ...
        if (cursorIndex == _highlightedIndex) {
          ...
          switch (_functionButton) {
            ...
            case kPageDownKey:
              ...
              break;
            case kExpandButton:
              [self setLocker:!_locked];
              [_view.textStorage replaceCharactersInRange:NSMakeRange(_view.pagingRange.location + _view.pagingRange.length / 2, 1)
                                     withAttributedString:_locked ? theme.symbolLock : _view.expanded ? theme.symbolCompress : theme.symbolExpand];
    ...
    case NSEventTypeMouseMoved: {
      ...
      if (cursorIndex >= 0 && cursorIndex < _indexRange.length && _highlightedIndex != cursorIndex) {
        ...
        if (theme.linear && _view.truncated[cursorIndex]) {
          [_toolTip showWithToolTip:[_view.textStorage.mutableString
                                     substringWithRange:_view.candidateRanges[cursorIndex]]
                           withDelay:NO];
        } else if (noDelay) {
          [_toolTip showWithToolTip:NSLocalizedString(@"candidate", nil) withDelay:!noDelay];
        }
    ...
    case NSEventTypeScrollWheel: {
      CGFloat scrollThreshold = theme.candidateParagraphStyle.minimumLineHeight +
...
- (void)highlightCandidate:(NSUInteger)highlightedIndex {
  ...
  for (NSUInteger i = 0; i < theme.pageSize; ++i) {
    ...
    if ((_sectionNum != priorSectionNum || priorIndex == priorHilitedIndex) && priorIndex < _indexRange.length) {
      NSRange priorRange = _view.candidateRanges[priorIndex];
      if (theme.linear && !_view.truncated[priorIndex]) {
        priorRange.length -= theme.tabular ? 3 : 2;
      }
      NSRange priorTextRange = [[_view.textStorage.mutableString substringWithRange:priorRange]
                               rangeOfString:_candTexts[priorIndex + _indexRange.location]];
      NSColor *labelColor = priorIndex == priorHilitedIndex && _sectionNum == priorSectionNum ?
                              theme.labelForeColor : theme.dimmedLabelForeColor;
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:labelColor
                                range:NSMakeRange(priorRange.location, priorTextRange.location)];
      if (priorIndex == priorHilitedIndex) {
        [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                  value:theme.textForeColor
                                  range:NSMakeRange(priorRange.location + priorTextRange.location,
                                                    priorTextRange.length)];
        [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                  value:theme.commentForeColor
                                  range:NSMakeRange(priorRange.location + NSMaxRange(priorTextRange),
                                                    priorRange.length - NSMaxRange(priorTextRange))];
      }
    }
    NSUInteger newIndex = i + _sectionNum * theme.pageSize;
    if ((_sectionNum != priorSectionNum || newIndex == _highlightedIndex) && newIndex < _indexRange.length) {
      NSRange newRange = _view.candidateRanges[newIndex];
      if (theme.linear && !_view.truncated[newIndex]) {
        newRange.length -= theme.tabular ? 3 : 2;
      }
      NSRange newTextRange = [[_view.textStorage.mutableString substringWithRange:newRange]
                              rangeOfString:_candTexts[newIndex + _indexRange.location]];
      NSColor *labelColor = newIndex == _highlightedIndex ? theme.hilitedLabelForeColor : theme.labelForeColor;
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:labelColor
                                range:NSMakeRange(newRange.location, newTextRange.location)];
      NSColor *textColor = newIndex == _highlightedIndex ? theme.hilitedTextForeColor : theme.textForeColor;
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:textColor
                                range:NSMakeRange(newRange.location + newTextRange.location, newTextRange.length)];
      NSColor *commentColor = newIndex == _highlightedIndex ? theme.hilitedCommentForeColor : theme.commentForeColor;
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:commentColor
...
- (void)highlightFunctionButton:(SquirrelIndex)functionButton
  ...
  switch (_functionButton) {
    case kPageUpKey:
      ...
      [_toolTip showWithToolTip:NSLocalizedString(_pageNum == 0 ? @"home" : @"page_up", nil) withDelay:delay];
      break;
    case kPageDownKey:
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:theme.hilitedPreeditForeColor
      ...
      [_toolTip showWithToolTip:NSLocalizedString(_finalPage ? @"end" : @"page_down", nil) withDelay:delay];
      break;
    case kExpandButton:
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:theme.hilitedPreeditForeColor
      ...
      [_toolTip showWithToolTip:NSLocalizedString(_locked ? @"unlock" : _view.expanded ?
                                                  @"compress" : @"expand", nil) withDelay:delay];
      break;
    case kBackSpaceKey:
      [_view.textStorage addAttribute:NSForegroundColorAttributeName
                                value:theme.hilitedPreeditForeColor
...
- (void)updateDisplayParameters {
  ...
  _textWidthLimit = floor((theme.vertical ? NSHeight(screenRect) : NSWidth(screenRect)) * textWidthRatio -
  ...
  if (theme.tabular) {
    _textWidthLimit = floor((_textWidthLimit + theme.fullWidth) / (theme.fullWidth * 2)) *
                        (theme.fullWidth * 2) - theme.fullWidth;
  }
  CGFloat textHeightLimit = floor((theme.vertical ? NSWidth(screenRect) : NSHeight(screenRect)) * 0.8 -
...
- (void)show {
  ...
  if (theme.rememberSize && _statusMessage == nil) {
    if (theme.lineLength < 0.1 && (theme.vertical
        ? (sweepVertical ? (NSMinY(_IbeamRect) - fmax(NSWidth(contentRect), _maxSize.width) - insets.right < NSMinY(screenRect))
        ...
        : (sweepVertical ? (NSMinX(_IbeamRect) - kOffsetGap - NSWidth(screenRect) * textWidthRatio - insets.left - insets.right >= NSMinX(screenRect))
                         : (NSMaxX(_IbeamRect) + fmax(NSWidth(contentRect), _maxSize.width) + insets.right > NSMaxX(screenRect))))) {
    ...
    CGFloat textHeight = fmax(NSHeight(contentRect), _maxSize.height) + insets.top + insets.bottom;
  ...
  if (_statusMessage != nil) { 
    ...
    if (theme.vertical) {
      ...
      windowRect = NSMakeRect(NSMaxX(self.frame) - NSHeight(contentRect) - insets.top - insets.bottom,
                              NSMaxY(self.frame) - NSWidth(contentRect) - insets.left - insets.right,
      ...
      windowRect = NSMakeRect(NSMinX(self.frame),
                              NSMaxY(self.frame) - NSHeight(contentRect) - insets.top - insets.bottom,
                              NSWidth(contentRect) + insets.left + insets.right,
                              NSHeight(contentRect) + insets.top + insets.bottom);
      _initPosition |= NSIntersectsRect(windowRect, _IbeamRect);
      if (_initPosition) {
  ...
  if (_view.preeditRange.length > 0) {
    ...
    if (theme.vertical != sweepVertical) {
      CGFloat anchorOffset = NSHeight([_view blockRectForRange:_view.preeditRange]);
      if (theme.vertical) {
...
- (void)setCandidateAtIndex:(NSUInteger)index
  ...
  if (text == nil) {
    if (index < _candTexts.count) {
      NSRange removeRange = NSMakeRange(index, _candTexts.count - index);
      [_candTexts removeObjectsInRange:removeRange];
      [_candComments removeObjectsInRange:removeRange];
    }
    return;
  }
  if (index >= _candTexts.count ||
      ![text isEqualToString:_candTexts[index]]){
    _candTexts[index] = text;
  }
  if (index >= _candComments.count ||
      ![(comment ? : @"") isEqualToString:_candComments[index]]) {
    _candComments[index] = comment ? : @"";
  }
}

- (NSUInteger)numCachedCandidates {
  return _candTexts.count;
}
...
- (void)showPreedit:(NSString *)preeditString
  ...
  if ((indexRange.length == 0 && preeditString && _view.preeditRange.length > 0) ||
      !updateCandidates) {
    rulerAttrsPreedit = [text attribute:NSParagraphStyleAttributeName
                                atIndex:0
                         effectiveRange:NULL];
  }
  if (updateCandidates) {
    ...
    _highlightedIndex = highlightedIndex;
    _view.candidateRanges = indexRange.length > 0 ? new NSRange[indexRange.length] : NULL;
    _view.truncated = indexRange.length > 0 ? new BOOL[indexRange.length] : NULL;
  }
  ...
  // preedit
  if (preeditString) {
    NSMutableAttributedString *preedit = [NSMutableAttributedString.alloc
                                          initWithString:preeditString
                                          attributes:theme.preeditAttrs];
    [preedit.mutableString appendString:rulerAttrsPreedit ? @"\t" : kFullWidthSpace];
    ...
    if (updateCandidates) {
      [text appendAttributedString:preedit];
  ...
  // candidate items
  candidatesStart = text.length;
  for (NSUInteger idx = 0; idx < indexRange.length; ++idx) {
    NSUInteger col = idx % theme.pageSize;
    NSMutableAttributedString *cand = idx / theme.pageSize != _sectionNum
    ? theme.candidateDimmedTemplate.mutableCopy : idx == highlightedIndex
    ...
    NSRange enumRange = [cand.mutableString rangeOfString:@"%c"];
    [cand replaceCharactersInRange:enumRange withString:theme.labels[col]];
    NSRange textRange = [cand.mutableString rangeOfString:@"%@"];
    [cand replaceCharactersInRange:textRange
                        withString:_candTexts[idx + indexRange.location]];
    NSRange commentRange = [cand.mutableString rangeOfString:kTipSpecifier];
    if (_candComments[idx + indexRange.location].length > 0) {
      [cand replaceCharactersInRange:commentRange
                          withString:[@"\u00A0" stringByAppendingString:
                                      _candComments[idx + indexRange.location]]];
    } else {
      [cand deleteCharactersInRange:commentRange];
    }
    // parse markdown and ruby annotation
    [cand formatMarkDown];
    CGFloat annotationHeight = [cand annotateRubyInRange:NSMakeRange(0, cand.length)
    ...
    if (annotationHeight * 2 > theme.linespace) {
      ...
      [cand addAttribute:NSParagraphStyleAttributeName
                   value:theme.candidateParagraphStyle
                   range:NSMakeRange(0, cand.length)];
      if (idx > 0) {
        if (theme.linear) {
          BOOL truncated = _view.truncated[0];
          NSUInteger start = _view.candidateRanges[0].location;
          for (NSUInteger i = 1; i <= idx; truncated = _view.truncated[i++]) {
            if (i == idx || _view.truncated[i] != truncated) {
              NSUInteger end = i == idx ? text.length : _view.candidateRanges[i].location;
          ...
          [text addAttribute:NSParagraphStyleAttributeName
                       value:theme.candidateParagraphStyle
                       range:NSMakeRange(candidatesStart, text.length - candidatesStart)];
        }
      }
    }
    ...
    NSUInteger candStart = text.length;
    [text appendAttributedString:cand];
    // for linear layout, middle-truncate candidates that are longer than one line
    if (theme.linear && ceil(cand.size.width) > _textWidthLimit - theme.fullWidth) {
      _view.truncated[idx] = YES;
      _view.candidateRanges[idx] = NSMakeRange(candStart, text.length - candStart);
      if (idx < indexRange.length - 1 || theme.tabular || theme.showPaging) {
        [text.mutableString appendString:@"\n"];
      }
      [text addAttribute:NSParagraphStyleAttributeName
                   value:theme.truncatedParagraphStyle
                   range:NSMakeRange(candStart, text.length - candStart)];
    } else {
      _view.truncated[idx] = NO;
      _view.candidateRanges[idx] = NSMakeRange(candStart, cand.length + (theme.tabular ? 3 : theme.linear ? 2 : 0));
    }
  }
  ...
  if (theme.tabular || theme.showPaging) {
    ...
    if (theme.tabular) {
      paging = [NSMutableAttributedString.alloc initWithAttributedString:
                _locked ? theme.symbolLock : _view.expanded ? theme.symbolCompress : theme.symbolExpand];
    } else {
      ...
      if (theme.vertical) {
        ...
        paging = [NSMutableAttributedString.alloc initWithAttributedString:pageNumString];
      }
    }
    if (theme.showPaging) {
      [paging insertAttributedString:_pageNum > 0 ? theme.symbolBackFill : theme.symbolBackStroke atIndex:0];
      [paging.mutableString insertString:kFullWidthSpace atIndex:1];
      [paging.mutableString appendString:kFullWidthSpace];
      [paging appendAttributedString:_finalPage ? theme.symbolForwardStroke : theme.symbolForwardFill];
    }
    if (!theme.linear || !_view.truncated[indexRange.length - 1]) {
      [text appendAttributedString:theme.separator];
      if (theme.linear) {
        [text replaceCharactersInRange:NSMakeRange(text.length, 0) withString:@"\n"];
      }
    }
    pagingStart = text.length;
    if (theme.linear) {
  ...
  } else if (theme.linear && !_view.truncated[indexRange.length - 1]) {
    [text appendAttributedString:theme.separator];
  }
...
AdjustAlignment:
  [_view estimateBoundsForPreedit:preeditRange
                    numCandidates:indexRange.length
                           paging:pagingRange];
  CGFloat textWidth = fmin(fmax(NSMaxX(_view.contentRect) - _view.trailPadding, _maxSize.width), _textWidthLimit);
  // right-align the backward delete symbol
  if (preeditRange.length > 0 && 
      NSMaxX([_view blockRectForRange:NSMakeRange(preeditRange.length - 1, 1)]) < textWidth - 0.1) {
  ...
  if (pagingRange.length > 0 &&
    ...
    if (theme.linear) {
      [text replaceCharactersInRange:NSMakeRange(pagingStart, 1) withString:@"\t"];
      ...
      [text replaceCharactersInRange:NSMakeRange(pagingStart + 1, 1)
                          withString:@"\t"];
      [text replaceCharactersInRange:NSMakeRange(text.length - 2, 1)
                          withString:@"\t"];
      rulerAttrsPaging.tabStops =
    ...
    [text addAttribute:NSParagraphStyleAttributeName
                 ...
                 range:NSMakeRange(pagingStart, text.length - pagingStart)];
  }

  // text done!
  CGFloat topMargin = preeditString || theme.linear ? 0.0 : ceil(theme.linespace * 0.5);
  CGFloat bottomMargin = !theme.linear && indexRange.length > 0 && pagingRange.length == 0 ? floor(theme.linespace * 0.5) : 0.0;
  NSEdgeInsets insets = NSEdgeInsetsMake(theme.borderInsets.height + topMargin,
...
- (void)showStatus:(NSString *)message {
  ...
  NSEdgeInsets insets = NSEdgeInsetsMake(theme.borderInsets.height,
                                         theme.borderInsets.width + ceil(theme.fullWidth * 0.5),
